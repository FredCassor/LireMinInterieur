  ...
  df <- reactive({
    ...
    isolate({
      if (input$load==0) df <- NULL
        ...
        df <- tryCatch(read.csv((input$file)$datapath, header=input$header, sep=input$separator, dec=input$decimal, stringsAsFactor=FALSE, check.names = FALSE, skip = input$skip),
                      error = function(c) read.csv((input$file)$datapath, header=input$header, sep=input$separator, dec=input$decimal, stringsAsFactor=FALSE, check.names = FALSE,  skip = input$skip, fileEncoding="ISO8859-1"))  
